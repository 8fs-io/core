...
// generateCompletion generates text using the AI service
func (s *service) generateCompletion(ctx context.Context, prompt string, maxTokens int, temperature float64) (string, error) {
	// Build messages for the AI service
	messages := []ai.ChatMessage{
		{
			Role:    "user",
			Content: prompt,
	...
	genRequest := ai.TextGenerationRequest{
		Messages:    messages,
		MaxTokens:   maxTokens,
	...
	// Call the AI service
	response, err := s.aiService.GenerateText(ctx, genRequest)
	if err != nil {
		return "", fmt.Errorf("AI service generation failed: %w", err)
	}

	// Extract the response text
	if len(response.Choices) == 0 {
		return "", fmt.Errorf("no response choices received from AI service")
	}
